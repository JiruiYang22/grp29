...
icache命中时只需要一周期读，而没有缓存的情况下需要4周期，所以减少了192M*3=576M周期的访存时间。dcache读命中时只需要一周期，而没有缓存的情况下需要4周期，所以减少了64M*3=192M周期的访存时间。dcache写命中时需要写回内存，再读取内存替换缓存，所以实际上需要比没有缓存的情况下多花费64M*3=192M周期的时间。也就是说总共减少的时间只是icache带来的性能提升。
